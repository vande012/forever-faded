 ...
 */ const build = async ({ logger, cwd, tsconfig, ...options })=>{
    ...
    try {
        ...
        if (ctx.bundler === 'webpack') {
            const { build: buildWebpack } = await Promise.resolve().then(function () { return require('./build-C21nr7XO.js'); });
            await buildWebpack(ctx);
        } else if (ctx.bundler === 'vite') {
            const { build: buildVite } = await Promise.resolve().then(function () { return require('./build-BU2xrLNx.js'); });
            await buildVite(ctx);
        }
...
const develop = async ({ cwd, polling, logger, tsconfig, watchAdmin, ...options })=>{
     ...
     */ let bundleWatcher;
        ...
        if (watchAdmin) {
            ...
            await writeStaticClientFiles(ctx);
            if (ctx.bundler === 'webpack') {
                const { watch: watchWebpack } = await Promise.resolve().then(function () { return require('./watch-BXKhPzUU.js'); });
                bundleWatcher = await watchWebpack(ctx);
            } else if (ctx.bundler === 'vite') {
                const { watch: watchVite } = await Promise.resolve().then(function () { return require('./watch-MAB-y3uk.js'); });
...
//# sourceMappingURL=index-DgH0C52K.js.map
